...
CSV by design; the "Ordinary" (bulk) numbers match. The new "TrueFamilyMLE"
baseline refits the data-generating family back to the whole sample with MLE.
A small ``--ridge`` (default ``1e-6``) regularizes the selected bandwidth so
weakly dependent tail samples stay positive-definite instead of being skipped;
pass ``--ridge 0`` for the unregularized formula.
...
def set_bicop(family: str, params: dict[str, float]) -> pv.Bicop:
  ...
  if family == "tawn":
    return pv.Bicop(
      ...
      rotation=0,
      parameters=np.array(
        [[params["delta1"]], [params["delta2"]], [params["theta"]]], dtype=float
      ),
    )
  raise ValueError(f"Unknown family: {family}")
...
def _fit_true_family_mle(
  u_data: Tensor, family: str, rotation: int
) -> pv.Bicop:
  """Fit the simulated family back to the full sample via MLE."""
  family_map = {
    "clayton": pv.BicopFamily.clayton,  # ty: ignore[unresolved-attribute]
    "gumbel": pv.BicopFamily.gumbel,  # ty: ignore[unresolved-attribute]
    "student": pv.BicopFamily.student,  # ty: ignore[unresolved-attribute]
    "gaussian": pv.BicopFamily.gaussian,  # ty: ignore[unresolved-attribute]
    "tawn": pv.BicopFamily.tawn,  # ty: ignore[unresolved-attribute]
  }
  bicop = pv.Bicop(family=family_map[family], rotation=rotation)
  controls = pv.FitControlsBicop(parametric_method="mle")
  bicop.fit(np.asfortranarray(u_data.numpy()), controls=controls)
  return bicop
...
def run_combo(spec: ComboSpec) -> ComboResult:
  ...
  try:
    ...
    oracle = _fit_true_family_mle(u_data, spec.family, spec.rotation)
    c_oracle, r_oracle, h_oracle, p_oracle = _truth_targets(
      oracle, u_tail_grid, q, spec.rotation
    )

    ests = {
      "Ordinary": {"r": r_body, "h": h_body, "c": c_body},
      "Tail": {"r": tail.r(grid), "h": tail.h(grid), "c": tail.c(grid)},
      "TrueFamilyMLE": {"r": r_oracle, "h": h_oracle, "c": c_oracle},
    }
    ...
    p_hats = {
      "Ordinary": p_body,
      "Tail": tail.fit_.p,
      "TrueFamilyMLE": p_oracle,
    }

    # Parametric tail fits are extra baselines; a failed fit drops just that
    # model, not the cell.
...
def build_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument(
    "--output",
    type=Path,
    default=Path("results/sim_study_biv_tail_true_tawn.csv"),
  )
